...
EkfExample::EkfExample(ros::NodeHandle n, ros::NodeHandle pn)
{
  ...
  LatLon ref_point(42.48803, -83.295589, 180);		//use P3 north america backyard co-ordinate 42.488035, -83.295589
  ...
  sub_fix = n.subscribe("gps_fix", 1, &EkfExample::recvFix, this);
  sub_twist = n.subscribe("twist", 1, &EkfExample::recvTwist, this);
...
void EkfExample::recvFix(const sensor_msgs::NavSatFixConstPtr& msg)
{
  ...
  raw_gps_transform.stamp_ = ros::Time::now();
  raw_gps_transform.frame_id_ = "map";
  raw_gps_transform.child_frame_id_ = "base_footprint";
...
void EkfExample::timerCallback(const ros::TimerEvent& event)
{
  ...
  ekf_transform.child_frame_id_ = "kalman_filter_prediction";			//chnaged from laser to base_foot_print
